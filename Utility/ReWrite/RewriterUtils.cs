...
namespace JawTek.Web.Utility.ReWrite
{
    ...
    internal class RewriterUtils
    {
        ...
        internal static void RewriteUrl(HttpContext context, string sendToUrl, out string sendToUrlLessQString, out string filePath)
        {
            ...
            string sendToUrlLessPathInfo = sendToUrlLessQString;
            if (sendToUrlLessPathInfo.Contains(".ashx"))
            {
                sendToUrlLessPathInfo = sendToUrlLessQString.Substring(0,
                    sendToUrlLessQString.IndexOf(".ashx", 0) + 5);
                pathInfo = sendToUrlLessQString.Substring(sendToUrlLessQString.IndexOf(".ashx", 0) + 5);
            }

            filePath = String.Empty;
            filePath = context.Server.MapPath(sendToUrlLessPathInfo);

            // rewrite the path...
            context.RewritePath(sendToUrlLessPathInfo, pathInfo, queryString);
